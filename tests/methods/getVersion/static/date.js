...
describe('get versions by date', function() {
  it('gets versions of a simple model', function(done) {
    ...
    async.waterfall([
      ...
      function(p, cb) {
        self.Simple.getVersion(p.simple.id, moment()
  ...
  it('gets versions of a composite model', function(done) {
    ...
    async.waterfall([
      ...
      function(p, cb) {
        self.Composite.getVersion(p.composite.id, moment()
  ...
  it('gets versions of a composite model with an array', function(done) {
    ...
    async.waterfall([
      ...
      function(p, cb) {
        self.CompositeWithArray.getVersion(p.composite.id, moment()
          .subtract(2, 'days'), function(err, v1) {
            p.v1 = v1;
            cb(err, p);
          });
  ...
  it('gets version of a model with a reference', function(done) {
    ...
    async.waterfall([
      ...
      function(p, cb) {
        self.Reference.getVersion(p.reference.id, moment()
          .subtract(2, 'days'), function(err, v1) {
            p.v1 = v1;
            cb(err, p);
          });
  ...
  it('gets versions of a model with a presave hook', function(done) {
    ...
    async.waterfall([
      ...
      function(p, cb) {
        self.Presave.getVersion(p.presave.id, moment()
          .subtract(3, 'days'), function(err, version) {
            p.v1 = version;
            cb(err, p);
          });
